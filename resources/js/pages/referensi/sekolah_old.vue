...
<template>
  <VCard title="Data Sekolah">
            ...
            <VCol cols="12" v-if="list.show">
              <VRow>
                <VCol cols="12" md="3" class="d-flex align-items-center">
                  <label class="v-label text-body-2 text-high-emphasis" :for="`app-text-field-${list.id}`">{{ list.title
                    }}</label>
                </VCol>
                <VCol cols="12" md="9">
                  <VFileInput accept="image/*" v-model="list.value" :id="list.id" :label="list.title"
                    v-if="list.file" />
    ...
    <ShowAlert :color="notif.color" :icon="notif.icon" :title="notif.title" :text="notif.text" :disable-time-out="false"
      v-model:isSnackbarVisible="isAlertVisible"></ShowAlert>
  </VCard>
</template>
