...
const {
  ...
  query: {
    data: 'tugas-sesi',
    tugas_sesi_id: route.params.id,
  },
}))
const tugas_sesi = computed(() => getData.value)
const notif = ref({
  icon: null,
  title: null,
  text: null,
...
const inputData = ref({
  tugas_sesi_id: tugas_sesi.value.tugas_sesi_id,
  judul: tugas_sesi.value.judul,
  deskripsi: tugas_sesi.value.deskripsi,
})
const dokumen = ref([{
  id: 1,
  berkas: null,
...
watch(isSnackbarClicked, () => {
  if(isSnackbarClicked.value){
    if(dokId.value){
      ...
      router.push({ name: 'pelatihan-sesi-tugas-id', params: {id: tugas_sesi.value.sesi_latihan_id} })
    }
...
<template>
  <VCard :title="`Edit Tugas ${tugas_sesi.judul}`">
    <VForm ref="refForm" v-model="isFormValid" @submit.prevent="onSubmit">
      ...
      <VCardText class="mt-16">
        <VRow v-for="(dok, index) in tugas_sesi.dokumen" :id="dok.id" :key="dok.id">
          <VCol md="6">
            {{ dok.nama }}
          </VCol>
          <VCol md="4">
