...
const {
  ...
  query: {
    data: 'materi-sesi',
    materi_sesi_id: route.params.id,
  },
}))
const materi_sesi = computed(() => getData.value)
const notif = ref({
  icon: null,
  title: null,
  text: null,
...
const inputData = ref({
  materi_sesi_id: materi_sesi.value.materi_sesi_id,
  judul: materi_sesi.value.judul,
  deskripsi: materi_sesi.value.deskripsi,
})
const dokumen = ref([{
  id: 1,
  berkas: null,
...
const onSubmit = async() => {
  refForm.value?.validate().then(async({ valid }) => {
    if (valid) {
      ...
      postData.append('data', 'materi-sesi');
...
watch(isSnackbarClicked, () => {
  if(isSnackbarClicked.value){
    if(dokId.value){
      ...
      router.push({ name: 'pelatihan-sesi-materi-id', params: {id: materi_sesi.value.sesi_latihan_id} })
    }
...
<template>
  <VCard :title="`Edit Materi ${materi_sesi.judul}`">
    <VForm ref="refForm" v-model="isFormValid" @submit.prevent="onSubmit">
      ...
      <VCardText class="mt-16">
        <VRow v-for="(dok, index) in materi_sesi.dokumen" :id="dok.id" :key="dok.id">
          <VCol md="6">
            {{ dok.nama }}
          </VCol>
          <VCol md="4">
